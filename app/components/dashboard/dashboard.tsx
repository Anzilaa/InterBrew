      ...
      <div className="rounded-lg border border-white/20 bg-black/60 p-4 backdrop-blur-md text-white shadow-lg">
        ...
        <div className="grid grid-cols-7 gap-2 text-xs">
          {["S", "M", "T", "W", "T", "F", "S"].map((d, idx) => (
            <div key={`${d}-${idx}`} className="text-center opacity-70">
              {d}
            </div>
          ))}
          ...
          {days.map((d) => (
            <div key={d} className="h-10 flex items-center justify-center">
              <div className="relative w-8 h-8 flex items-center justify-center rounded-full bg-white/5">
                ...
                {pending.has(d) && (
                  <div className="absolute -right-2 -top-2 bg-red-600 rounded-full w-5 h-5 flex items-center justify-center text-[10px]">
                    ✕
                  </div>
                )}
                {confirmed.has(d) && (
                  <div className="absolute -right-2 -top-2 bg-emerald-500 rounded-full w-5 h-5 flex items-center justify-center text-[10px]">
                    ✓
                  </div>
                )}
              </div>
        ...
        <div className="-mx-2">
          <div className="flex gap-4 overflow-x-auto px-2 py-2 scrollbar-hide">
            {collections.map((c) => (
              <div
                key={c.title}
                className={`min-w-55 shrink-0 rounded-lg p-4 text-white ${c.color} shadow-lg`}
              >
            ...
            <div className="w-full">
              <div className="rounded-lg border border-gray-300 dark:border-gray-700 p-4 bg-transparent">
                ...
                <div className="text-sm leading-relaxed whitespace-pre-line text-gray-700 dark:text-gray-300">
                  Bro you suck at frontend dont you even dare to hop on next.js
                  before knowing basic css And have you seen your score in
                  verbal aptitude? Man oh man english ain't a second language to
                  you but a language from a second planet.
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-6">
              <div className="rounded-lg p-4 bg-white/90 dark:bg-gray-800/70 shadow w-56 border text-center">
                <div className="text-sm text-gray-600 dark:text-gray-300">
                  Slaying September Contest
                </div>
                <div className="mt-4">
                  <button className="px-4 py-2 rounded-md border bg-gray-100 dark:bg-gray-900/40">
                    Continue
                  </button>
                </div>
              </div>
              ...
              <div className="mx-auto w-full max-w-3xl h-full">
                ...
                <div className="relative h-[calc(100%-56px)]">
                  ...
                  >
                    ...
                    <path
                      d="M0 200 L50 160 L100 180 L150 120 L200 140 L250 100 L300 120 L350 90 L400 130 L450 80 L500 100 L550 60 L600 40"
                      fill="url(#g1)"
                      stroke="none"
                    />
                    <path
